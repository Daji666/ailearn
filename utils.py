...
_PAD = '<PAD>'  # 补全字符
_SOS = '<s>'    # start of sentence 句子起始标识符
_EOS = '</s>'   # end of sentence 句子结束标识符
_UNK = '<UNK>'  # OOV word 未登录词
...
class Vocab(object):
    ...
    def __init__(self, name):
        self.name = name
        self.trimmed = False    #是否已经去除低频词
        self.word2index = {}    #字符:索引 字典
        self.word2count = {}    #统计每个单词的出现次数
        self.index2word = {}    #索引:字符 字典
        self.num_words = 0
    ...
    def add_word(self, word):
        if word not in self.word2index: #第一次出现该词
            self.word2index[word] = self.num_words  #构建字典
    ...
    def trim(self, max_vocab_size=None):
        ...
        if config.trim_vocab_max_size:
            ...
            if self.num_words <= max_vocab_size:    # 总词数过少，则不剔除
                return
            for special_symbol in _START_VOCAB:
                self.word2count.pop(special_symbol) #剔除关键词 [_PAD, _SOS, _EOS, _UNK]
            keep_words = list(self.word2count.items())
            keep_words = sorted(keep_words, key=lambda item: item[1], reverse=True) #降序排列
            keep_words = keep_words[: max_vocab_size - len(_START_VOCAB)]   #剔除超过限制大小的词（优先剔除出现频率低的词）
            keep_words = _START_VOCAB + [word for word, _ in keep_words]    #只留下词,加上关键词，正好满足max_vocab_size
    ...
    def save(self, name):
        ...
        :return:
        """
        path = os.path.join(config.vocab_dir, name) #路径拼接函数
...
def load_dataset(dataset_path) -> list:
    ...
    with open(dataset_path, 'r', encoding='utf-8') as file:
        for line in file.readlines():
            words = line.strip().split(' ') #删除换行符，根据空格分词
            lines.append(words)
    return lines


def filter_data(codes, asts, nls):  #根据规则过滤数据
    """
    ...
    for i in range(len(codes)):
        ...
        ast = asts[i]
        nl = nls[i]
        if len(code) > config.max_code_length or len(nl) > config.max_nl_length or len(nl) < config.min_nl_length:  #超过设置的最大长度，则不读取该数据
            continue
...
def unsort_collate_fn(batch, code_vocab, ast_vocab, nl_vocab, raw_nl=False):
    ...
    # 将单词转移到索引中，包括oov单词，并将EOS标记附加到每个句子、列表中
    code_batch = indices_from_batch(code_batch, code_vocab)  # [B, T]
    ast_batch = indices_from_batch(ast_batch, ast_vocab)  # [B, T]
    if not raw_nl:
        nl_batch = indices_from_batch(nl_batch, nl_vocab)  # [B, T]
