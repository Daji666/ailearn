    ...
    for names in rnn._all_weights:
        for name in names:
            if name.startswith('weight_'):
                wt = getattr(rnn, name)
                wt.data.uniform_(-config.init_uniform_mag, config.init_uniform_mag) #在[-config.init_uniform_mag,config.init_uniform_mag)中随机生成
...
class Encoder(nn.Module):
    ...
    def __init__(self, vocab_size):
        ...
        self.embedding = nn.Embedding(vocab_size, config.embedding_dim) #实现词嵌入(所有单词数目， 词嵌入维度)
        self.gru = nn.GRU(config.embedding_dim, self.hidden_size, bidirectional=True)   #True->双向循环神经网络

        init_wt_normal(self.embedding.weight)   #手动设置权重标准差
        init_rnn_wt(self.gru)   #根据设置的参数，随机初始化rnn参数
    ...
    def init_hidden(self, batch_size):  #隐藏状态
...
class ReduceHidden(nn.Module):
    ...
    def __init__(self, hidden_size=config.hidden_size):
        ...
        init_linear_wt(self.linear) #初始化参数
...
class Decoder(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, last_hidden: torch.Tensor,
        ...
        code_context = code_context.transpose(0, 1)     # [1, B, H] 0,1维交换
...
class Model(nn.Module):
    ...
    def __init__(self, code_vocab_size, ast_vocab_size, nl_vocab_size,
                 model_file_path=None, model_state_dict=None, is_eval=False):   # model_state_dict 保存模型中的weight权值和bias偏置值
